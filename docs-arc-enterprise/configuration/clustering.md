...
Both `catchup_failed` and `catchup_dropped` are surfaced in the 503 body so operators see exactly what happened. The `/api/v1/cluster/status` endpoint also exposes the cumulative `total_failed` / `total_dropped` counters alongside the catch-up-scoped values, so dashboards can distinguish "the catch-up batch had a hiccup" from "the puller has been having steady-state problems for hours."

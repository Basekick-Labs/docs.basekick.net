...
arc:
  storageBackend: local
  # WAL is on by default in the chart; shown here for clarity.
  wal:
    enabled: true
  env:
    - name: ARC_AUTH_ENABLED
...
migrations, either size the volume for the burst or set `arc.wal.enabled: false`
for the migration window and re-enable it afterwards — accepting that a crash
during that window loses whatever is buffered.
...
arc:
  ...
  # Write-ahead log. On by default in the chart; the Arc binary itself
  # defaults wal.enabled to false for backwards compatibility, so the chart
  # overrides it. Without a WAL a crash loses every record buffered since the
  # last flush.
  wal:
    enabled: true
...
- [Monitoring & ops playbook](/arc/operations/monitoring/) — what to scrape and alert on
- [Telemetry](/arc/operations/telemetry/) — what Arc reports back, and how to opt out

...
description: "Step-by-step guide to migrating from InfluxDB (1.x, 2.x, 3.x) to Arc: re-point Line Protocol ingestion, bulk-load history with tsm2arc, and translate InfluxQL and Flux to standard SQL."
---
...
- **History has a purpose-built tool.** [`tsm2arc`](https://github.com/Basekick-Labs/tsm2arc) reads InfluxDB storage directly — 1.x/2.x TSM files off disk, 3.x parquet object stores from disk or S3 — and loads Arc, reconstructing multi-field points correctly. Line Protocol export/import remains a fallback for every version.
...
<Tabs items={['Native (macOS)', 'Native (Linux)', 'Docker']}>
  <Tab value="Native (macOS)">
...
<Tabs items={['InfluxDB 1.x', 'InfluxDB 2.x', 'InfluxDB 3.x']}>
  <Tab value="InfluxDB 1.x">
...
<Callout type="warn" title="`influx_inspect export` splits fields into separate lines">
...
### Bulk migration with tsm2arc (recommended)

[tsm2arc](https://github.com/Basekick-Labs/tsm2arc) (Apache-2.0) reads InfluxDB storage **directly** — 1.x/2.x TSM and WAL files off disk, and 3.x parquet object stores from local disk or **in place on S3** — reconstructs each multi-field point into a single Line Protocol record, and streams it into Arc's `/api/v1/import/lp` endpoint with resumable checkpointing. It does not need a running InfluxDB, which makes it ideal for cold volumes, EBS snapshots, and orphaned S3 buckets. For 3.x it reads every catalog format natively and recovers un-snapshotted WAL (up to ~10 minutes of the newest writes live only there — a clean v3 shutdown does not flush them); see [InfluxDB 3 sources](/arc/migration/tsm2arc/influxdb3/) for the v3 specifics and the Enterprise/Pacha support matrix. The Line Protocol path above remains the fallback for the out-of-scope 3.x cases.
...
<Callout type="idea" title="tsm2arc operational notes">
...
<Callout type="info" title="Percentiles differ slightly by design">

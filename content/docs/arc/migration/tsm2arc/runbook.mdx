...
tsm2arc auto-detects 1.x vs 2.x vs 3.x; for 1.x/2.x the TSM/WAL file format is identical, only the directory layout differs.
...
InfluxDB 3.x is a parquet object store, not TSM — point `--datadir` at the store root (a local directory or an `s3://` URL) and see [InfluxDB 3 sources](/arc/migration/tsm2arc/influxdb3/) for the v3-specific behavior (native WAL recovery, catalogs, Enterprise tiers). Everything else on this page — dry run, sizing, resume, verification — applies to 3.x unchanged.
...
Text output shows the five largest runs per shard and says so when it truncates; `--analyze-runs=0` prints everything. For tooling, use `--format=json`: one JSON document with every run of every shard and nothing else on stdout. Add `--redact` to replace database, retention policy, and series names with stable hashed identifiers when the report must leave your organization.
...
Under the default `--on-invalid-measurement=fail`, a load runs a pre-flight census: every measurement name in the (time-windowed) corpus is checked against your map **before the first POST**, and a failure lists every offending name at once, so one map fix covers all of them. The census reads only TSM indexes and WAL keys; it does not decode data.
...
## If a writer restarts mid-load

A rolling restart of Arc (or anything that severs keep-alive connections, such as an L7 proxy in front of the writers) makes the in-flight POST fail fast and retry, visibly: requests send `Expect: 100-continue` so a dead backend usually fails the attempt in about a second, each attempt has a deadline (`--send-timeout`, auto-derived from `--chunk-bytes`), every failed attempt logs one line, and retries open a fresh connection. The heartbeat shows the oldest shard's idle age and the last send failure, and warns once a shard exceeds `--stall-warn`. When the whole retry budget is spent the process exits non-zero, and a plain re-run resumes exactly where it left off.

Avoid setting `--send-timeout` aggressively low: the deadline covers Arc's flush-before-ack, and a timeout that races a *successful* import re-sends the chunk (tag-bearing series dedupe at compaction; tagless rows can keep the duplicate).

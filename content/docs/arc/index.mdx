---
title: "Arc OSS"
description: "Arc is an open, SQL-native time-series database on DuckDB and Parquet: ingest MessagePack or Line Protocol, query with standard SQL, and store data on local disk, S3, MinIO, or Azure Blob."
---

**Open, SQL-native time-series database built on DuckDB and Parquet**

Arc is a SQL-native time-series database built on DuckDB and Parquet with flexible storage backends. Use it for analytics, observability, AI/ML, IoT, and log management. High-throughput columnar ingestion, fast analytical queries. Single Go binary. S3/Azure native. No vendor lock-in. AGPL-3.0.
...
{/* TODO(diagram): replace the ASCII box-flow below with a real diagram of the write path (client -> API -> buffer -> Parquet -> object storage) and the read path (DuckDB querying Parquet in place). The separation of compute and storage is Arc's central design claim and deserves better than monospace arrows. */}

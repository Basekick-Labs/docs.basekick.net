...
description: "Deploy an Arc Enterprise cluster with Helm: separate writer, reader, and compactor StatefulSets, IRSA or MinIO storage, peer replication, and an L7 load balancer for writers."
---
...
<Tabs items={['Local (Peer Replication)', 'AWS S3', 'MinIO']}>
  <Tab value="Local (Peer Replication)">
...
<Callout type="idea" title="IRSA requires Arc 26.06.2 or later for query reads">
...
<Tabs items={['Shared Storage (MinIO)', 'Local Storage (Peer Replication)', 'External S3']}>
  <Tab value="Shared Storage (MinIO)">
...
<Callout type="warn" title="Required overrides">
...
the chart refuses to install if any of them is empty.
  ...
  <Tab value="Local Storage (Peer Replication)">
...
<Callout type="idea" title="Replication tuning">
...
<Callout type="warn" title="Multi-writer needs an L7 load balancer in front of the writer Service">
...
For what to actually watch — per-role expectations, cluster and replication
signals, failover, and the metrics that are exported but never populated — see
the [Monitoring & Ops Playbook](/arc-enterprise/operations/monitoring/).

### Prometheus metrics

Arc exposes Prometheus metrics at `/metrics`. The endpoint is **unauthenticated
by design**, because Prometheus scrapers expect an unauthenticated target;
restrict it at your ingress if Arc is reachable beyond the cluster.

The chart does not ship a `ServiceMonitor`, so create one yourself:
...
The chart also sets a `startupProbe` with a 30-failure threshold (five minutes).
Keep it: WAL replay runs before the HTTP listener binds, so without a startup
gate a long recovery looks like a liveness failure and the pod crash-loops.

<Callout type="warn" title="The two probes are not interchangeable">
`/health` is **liveness only** and always returns 200 — deliberately, because
restarting a pod does not fix expired storage credentials, it just loops the
node through restarts. Alert on the `storage` block in its response body rather
than on the status code.

`/ready` is the one that returns 503, in exactly two cases: the node is starting
up or shutting down, or storage credentials have expired **and**
`server.storage_credentials_fail_ready` is `true`. That key defaults to `false`,
so enable it on reader pools you want drained when credentials lapse. See the
[Monitoring & Ops Playbook](/arc-enterprise/operations/monitoring/).
</Callout>
...
| `3` | HA + horizontal scale. Writes round-robin across all healthy pods, and losing one still leaves a spare. The chart default. |
| `2` | **Refused by chart validation** — the second writer is the only spare, so one failure consumes it and no writer can be drained for a rolling upgrade. |

Raft quorum follows `writer.replicas` here, because only nodes that accept
writes vote. In a deployment of writers, readers and a compactor the voters are
the writer pods; the others replicate the log and see all cluster state but
never campaign. That is why `2` is refused on both counts at once: two writers
are two voters, so losing one costs the quorum as well as the spare.

A cluster upgraded in place keeps any pod that did not shut down gracefully as
a voter until it is removed and re-joins, so the voter set converges over a
rolling restart rather than at the moment of upgrade.
...
- [Monitoring & ops playbook](/arc-enterprise/operations/monitoring/) — cluster, replication and failover signals
- [Telemetry](/arc-enterprise/operations/telemetry/) — what Arc reports back, and how to opt out

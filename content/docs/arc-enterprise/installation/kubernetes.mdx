...
Raft quorum is a separate, cluster-wide property: every node that joins becomes
a voter regardless of its role, so readers and the compactor carry quorum too.
Size the cluster for an odd total pod count if Raft tolerance matters to you,
and size `writer.replicas` for ingest availability.
